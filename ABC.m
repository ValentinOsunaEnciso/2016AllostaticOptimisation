...
%*/
function GlobalMin=ABC(funcion,D,mefo)
%clear all
%format long
...
ub=ones(1,D)*r(2,1); %/*lower bounds of the parameters. */
lb=ones(1,D)*r(1,1);%/*upper bound of the parameters.*/
...
for ind5=1:FoodNumber
    ObjVal(ind5)=FUNCION(Foods(ind5,:),D,funcion); EvalFO=EvalFO+1;
end
...
iter=0;
while EvalFO<mefo%k<maxIter ((iter < maxCycle)),
...
%%%%%%%%% EMPLOYED BEE PHASE %%%%%%%%%%%%%%%%%%%%%%%%
    for i=1:(FoodNumber)
       ...
       %  /*if generated parameter value is out of boundaries, it is shifted onto the boundaries*/
        ...
        ObjValSol=FUNCION(sol,D,funcion);
        EvalFO=EvalFO+1;
...
iter=iter+1;
CONVERGENCIA(iter)=GlobalMin;
end % End of ABC
%GlobalMins(r)=GlobalMin;
%end; %end of runs
%semilogy(CONVERGENCIA,'--k','LineWidth',2)
corridas(ind1)=GlobalMin;
%fprintf('Corrida=%d ObjVal=%e, EvalFO=%d\n',ind1,corridas(ind1),EvalFO);
%end
%disp(sprintf('ABC:Func:%d,EvalFO:%d,Best:%0.3e\n',funcion,EvalFO,GlobalMin));
end
%save all
